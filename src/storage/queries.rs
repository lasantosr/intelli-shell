...
) -> Result<SelectStatement, SearchError> {
...
fn query_commands_filtered(
    ...
    tuning: &SearchCommandTuning,
    workspace_tables_loaded: bool,
) -> Result<(WithClause, SelectStatement, bool), SearchError> {
...
fn query_commands_with_text_search(
...
) -> Result<(Vec<CommonTableExpression>, SelectStatement, bool), SearchError> {
    ...
    let (final_query, has_rank) = match search_mode {
        ...
        SearchMode::Regex => {
            let regex = Regex::new(&term).map_err(SearchError::InvalidRegex)?;
        ...
        SearchMode::Fuzzy => {
            ...
            if fuzzy_matches.is_empty() {
                return Err(SearchError::InvalidFuzzy);
            }
